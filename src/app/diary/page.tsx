...
export default function DiaryPage() {
  ...
  const [expanded, setExpanded] = useState<string | null>('2026-05-19')
  ...
  useEffect(() => {
    fetch('/data/diary_index.json')
      .then(r => r.json())
      .then(data => {
        setDiaries(data.reverse()) // newest first
        setLoading(false)
        // Load latest diary by default
        const latest = data[data.length - 1]?.date
        if (latest) loadDiary(latest)
      })
  }, [])
  ...
  function renderBlocks(entries: { type: string; text: string }[]) {
    return entries.map((block, i) => {
      ...
      switch (block.type) {
        case 'heading_1':
          return <h2 key={key} className="text-2xl font-bold text-pink-700 mt-6 mb-3 pb-1 border-b border-pink-200">{block.text}</h2>
        case 'heading_2':
          return <h3 key={key} className="text-lg font-semibold text-pink-600 mt-5 mb-2">{block.text}</h3>
        case 'heading_3':
          return <h4 key={key} className="text-base font-medium text-pink-500 mt-4 mb-1">{block.text}</h4>
        case 'callout':
          return <div key={key} className="bg-pink-50 border-l-4 border-pink-400 p-3 my-3 rounded-r-lg text-pink-800">{block.text}</div>
        case 'quote':
          return <blockquote key={key} className="border-l-4 border-pink-300 pl-4 my-3 italic text-gray-600">{block.text}</blockquote>
        case 'bulleted_list_item':
          return <li key={key} className="ml-5 list-disc text-gray-700 my-1">{parseBold(block.text)}</li>
        case 'numbered_list_item':
          return <li key={key} className="ml-5 list-decimal text-gray-700 my-1">{parseBold(block.text)}</li>
        case 'to_do':
          return <div key={key} className="flex items-start gap-2 my-1 text-gray-700">☐ {parseBold(block.text)}</div>
        case 'divider':
        ...
        default:
          if (block.text.startsWith('**') || block.text.includes('**')) {
            return <p key={key} className="text-gray-700 my-2 leading-relaxed">{parseBold(block.text)}</p>
          }
          return <p key={key} className="text-gray-700 my-2 leading-relaxed">{block.text}</p>
      }
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-pink-50 via-white to-white">
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <p className="text-gray-500 mb-10 ml-11">
              ...
              <div key={diary.date} className="border border-pink-100 rounded-xl overflow-hidden transition-all duration-200 hover:border-pink-200">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-gray-800 font-medium">{displayDate(diary.date)}</span>
                  </div>
                  <ChevronRight
